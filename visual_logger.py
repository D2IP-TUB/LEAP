...
def find_step_csv(log_dir: str, full_request_id: str, step: int) -> str | None:
    for csv_file in os.listdir(log_dir):
        ...
        if (
            step < 10
            and f"step{step}" in csv_file
            and f"step{step:02d}" not in csv_file
        ):
            if (
                f"step{step}_" in csv_file
                or f"step{step}." in csv_file
                or csv_file.endswith(f"step{step}.csv")
            ):
                return csv_file
...
def process_log_file(
    log_path: str, results_data: Dict[str, Dict[str, Any]], log_dir: str
) -> List[Dict[str, Any]]:
    try:
