...
class VLLMWorkerProcess(mp.Process):
    ...
    def __init__(
        ...
        self.constraint_processors = self.generation_config.get(
            "constraint_processors", {}
        )
        self.generation_functions = self.generation_config.get(
    ...
    async def _process_requests(self):
        ...
        while True:
            try:
                ...
                to_delete = [
                    key for key in state_machines if key.startswith(request_id)
                ]
    ...
    async def _process_single_request(
        self, request: Dict[str, Any], state_machines: Dict
    ) -> Dict[str, Any]:
        """
...
class ProcessParallelVLLM:
    ...
    def _create_workers(self):
        ...
        tensor_parallel_size = self.generation_config["engine_config"][
    ...
    def generate_batch(
        self, requests: List[Dict[str, Any]], timeout_per_request: int = 180
    ) -> List[Dict[str, Any]]:
        """
        ...
        return [
            results.get(req_id, {"error": "Request not completed"})
...
def setup_standard_vllm_server(
    ...
    generation_config = create_generation_config(
        use_constraints=use_constraints, use_cot=use_cot, logging_config=logging_config
    )
