...
async def evaluate_instance(
    ...
    count_text = f"{len(most_common_sem_examples)}/{total}"
    print(f"    Most common semantic rows: {list(most_common_sem_rows)} ({count_text} = {semantic_consistency:.2%})")
...
async def main():
    ...
    for r in all_results:
        ...
        tokens_in = r["token_usage"]["total_input_tokens"]
        tokens_out = r["token_usage"]["total_output_tokens"]
        tokens_total = r["token_usage"]["total_tokens"]
        print(f"    Tokens: {tokens_in:,} in / {tokens_out:,} out / {tokens_total:,} total")
        sem_pct = r["semantic_consistency"]["consistency_percentage"]
        sem_resp = r["semantic_consistency"]["most_common_response"]
        print(f"    Semantic consistency: {sem_pct:.2%} (most common: {sem_resp})")
        pos_pct = r["positional_bias"]["consistency_percentage"]
        pos_resp = r["positional_bias"]["most_common_response"]
        print(f"    Positional consistency: {pos_pct:.2%} (most common: {pos_resp})")
