...
def build_dynamic_plan_prompt(
    model_config, question: str, table: Dict[str, Any], action_history: List[str]
) -> str:
    ...
    prompt = model_config.add_instruct_tokens_for_instruct_models(
...
def build_generate_args_prompt(
    model_config,
    question: str,
    table: Dict[str, Any],
    action_name: str,
...
def extract_arguments_from_text(
    text: str, action_name: str, table: Dict[str, Any]
) -> Optional[List]:
...
async def generate_single_action(
    worker, prompt, table, request_id, state_machines, action_history=None
):
    """
    ...
    try:
        if worker.use_constraints:
            ...
            use_global_constraints = worker.generation_config.get(
...
async def generate_action_arguments(
    ...
    prompt = build_generate_args_prompt(
        model_config, question, table, action_name, action_history
    )
    ...
    if estimated_length > worker.max_model_len - 100:
        ...
        if action_name == "select_row":
            ...
            instruction_prompt = (
                f"Which columns from {table['columns'][:3]}...?\nColumn names: "
            )
